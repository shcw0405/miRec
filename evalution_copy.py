# 只需要把train.py里面的evalution修改成evalution copy就行了
...
def calculate_discrimination(history_cate_count, rec_cate_count):
    ...
    参数:
    history_cate_count (dict): 用户历史行为中各类别的计数
    rec_cate_count (dict): 推荐结果中各类别的计数
    
    返回:
    float: 歧视指标得分，越高表示歧视越严重
    dict: 各类别比例变化的详细信息
    """
    # 计算历史行为中各类别的比例
    total_history = sum(history_cate_count.values())
    if total_history == 0:
        return 0.0, {}
    
    history_cate_ratio = {cate: count / total_history for cate, count in history_cate_count.items()}
    
    # 计算推荐结果中各类别的比例
    total_rec = sum(rec_cate_count.values())
    if total_rec == 0:
        return 0.0, {}
    
    rec_cate_ratio = {cate: count / total_rec for cate, count in rec_cate_count.items()}
    
    # 计算所有类别的比例变化
    all_categories = set(list(history_cate_ratio.keys()) + list(rec_cate_ratio.keys()))
    category_changes = {}
    
    for cate in all_categories:
        history_ratio = history_cate_ratio.get(cate, 0)
        rec_ratio = rec_cate_ratio.get(cate, 0)
        
        # 计算比例变化，可能为正(增强)或负(歧视)
        change = rec_ratio - history_ratio
        category_changes[cate] = {
    ...
    # 计算歧视得分：少数类别被歧视的程度
    # 这里我们定义少数类别为历史行为中比例低于平均水平的类别
    avg_history_ratio = 1.0 / len(history_cate_ratio) if history_cate_ratio else 0
    
    # 歧视得分：少数类别的比例下降总和
    discrimination_score = 0.0
    for cate, change_info in category_changes.items():
        if change_info['history_ratio'] < avg_history_ratio:  # 少数类别
            if change_info['change'] < 0:  # 比例下降，被歧视
                discrimination_score += abs(change_info['change'])
    
    return discrimination_score, category_changes
